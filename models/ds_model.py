...
class PointCloudAE(nn.Module):
    def __init__(self, point_size=1024, latent_dim=128, hidden_dim=256, dropout=1e-3):
        super().__init__()
    ...
    def encode(self, x, edge_index):
        #print(x.shape)
        num_nodes = x.shape[1]  # Should be 1024
    ...
    def forward(self, x, edge_index):
        ...
        valid_mask = (edge_index < num_nodes).all(dim=0)  # Shape: [num_edges]
        edge_index = edge_index[:, valid_mask]  # Keep only valid edges
        return self.decode(self.encoder(x.to('mps'), edge_index.to('mps')))
...
class PointCloudAutoencoder(pl.LightningModule):
    ...
    def encode(self, batch):
        return self.net.encode(batch.pos.permute(0,2,1), batch.edge_index)
